...
jd_t khronos::gregorian_to_jd(year_t year, month_t month, day_t day) {
...
jd_t khronos::gregorian_to_jd(year_t year, month_t month, day_t day, hour_t hour, minute_t minute, second_t seconds) {
	assert(year > -4800 && "Minimum year is 4800 BCE");
	assert(month >= 1 && "Minimum month is January = 1");
	assert(month <= 12 && "Maximum month is December = 12");
	assert(day >= 1 && "Minimum day of the month is 1");
...
void khronos::jd_to_gregorian(jd_t jd, year_t& year, month_t& month, day_t& day) {
...
void khronos::jd_to_gregorian(jd_t jd, year_t& year, month_t& month, day_t& day, hour_t& hour, minute_t& minute, second_t& seconds) {
...
Gregorian khronos::operator + (Gregorian const& dt, detail::packaged_year_integer const& year) {
	year_t y = dt.year() + year.nYears_;
	month_t m = dt.month();
	day_t d = dt.day();

	if (m == FEB && d == 29 && !is_gregorian_leapyear(y))
		d = 28;
...
Gregorian khronos::operator + (Gregorian const& dt, detail::packaged_month_integer const& month) {
