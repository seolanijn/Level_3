/*	ut_Khronos_101-102,119_vulcan_conv.cpp
...
#if PHASE_101
TEST_PHASE(ut101_test_vulcan_to_jd,1) {
...
#else
EMPTY_PHASE(ut101_test_vulcan_to_jd,1)
#endif
...
TEST_PHASE(ut102_test_jd_to_vulcan,1) {
	using namespace khronos;
...
#else
EMPTY_PHASE(ut102_test_jd_to_vulcan,1)
#endif


#if PHASE_119 && defined( FULL_TEST )
/**	Check all conversions from the lowest Proleptic Vulcan date, until a day well in the future. */
TEST_PHASE(ut119_test_vulcan_to_jd_full,1) {
	using namespace khronos;
	...
	for (jd_t jd = low; jd <= high; jd += increment) {
		...
		GATS_CHECK_CLOSE_EPSILON(jd, result, 0.000'000'1);
	}
	cout << "... completed\n";
}
#else
EMPTY_PHASE(ut119_test_vulcan_to_jd_full,1)
#endif